...
export default function BookingConfirmation({ params }: { params: { id: string } }) {
  ...
  const fetchTicketInfo = async () => {
    try {
      const response = await axios.get(`http://localhost:5000/api/user/get-ticket/${params.id}`);
  ...
  const handleWhatsappSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await axios.post('http://localhost:5000/api/user/send-whatsapp', { ticketId: params.id, whatsappNumber });
      alert('Booking details sent to your WhatsApp!');
    } catch (error) {
        ...
        <div className="bg-white shadow-lg rounded-lg overflow-hidden">
          ...
          <div className="p-6">
            <div className="flex items-center mb-4">
              ...
              <div>
                <p className="font-semibold text-lg">{ticketInfo.movieName}</p>
                <p className="text-sm text-gray-600">{new Date(ticketInfo.showDateTime).toLocaleString()}</p>
              </div>
            </div>
            
            <div className="flex items-center mb-4">
              <MdTheaters className="text-3xl text-green-500 mr-3" />
              <p className="font-semibold">{ticketInfo.theaterName}</p>
            </div>
            
            <div className="flex items-center mb-4">
              <MdEventSeat className="text-3xl text-green-500 mr-3" />
              <p className="font-semibold">Seats: {ticketInfo.seats.join(', ')}</p>
            </div>
            
            <div className="mt-6 flex justify-center">
              <Image src={ticketInfo.qrCode} alt="Ticket QR Code" width={200} height={200} />
            </div>
          ...
          <div className="bg-gray-50 px-6 py-4">
            <form onSubmit={handleWhatsappSubmit} className="flex items-center">
              <input
                type="tel"
